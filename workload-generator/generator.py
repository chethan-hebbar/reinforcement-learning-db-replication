...
# We will now cycle through 6 distinct profiles every 10 seconds.
PHASE_DURATION_SECONDS = 5

# Profile 1: Key 0 is viral in the US
profile_1 = {
    "key_distribution": [0.80, 0.05, 0.05, 0.05, 0.05], # 80% for key 0
    "region_distribution": [0.7, 0.2, 0.1],             # 70% from US
    "read_write_ratio": 0.95
}

# Profile 2: Key 3 becomes popular in Europe
profile_2 = {
    "key_distribution": [0.05, 0.05, 0.1, 0.75, 0.05], # 75% for key 3
    "region_distribution": [0.1, 0.8, 0.1],             # 80% from EU
    "read_write_ratio": 0.90
}

# Profile 3: Key 1 is now the focus, primarily in Asia-Pacific
profile_3 = {
    "key_distribution": [0.05, 0.80, 0.05, 0.05, 0.05], # 80% for key 1
    "region_distribution": [0.1, 0.2, 0.7],             # 70% from AP
    "read_write_ratio": 0.95
}

# Profile 4: Key 4 is read-heavy, globally distributed
profile_4 = {
    "key_distribution": [0.05, 0.05, 0.05, 0.05, 0.80], # 80% for key 4
    "region_distribution": [0.3, 0.4, 0.3],             # Balanced regional traffic
    "read_write_ratio": 0.98 # Very high read ratio
}

# Profile 5: Key 2 has a spike in writes from the US (e.g., a software update)
profile_5 = {
    "key_distribution": [0.05, 0.05, 0.80, 0.05, 0.05], # 80% for key 2
    "region_distribution": [0.7, 0.1, 0.2],             # 70% from US
    "read_write_ratio": 0.60 # Much higher write ratio
}

# Profile 6: A "balanced" or "chaotic" state with no single dominant key
profile_6 = {
    "key_distribution": [0.25, 0.15, 0.20, 0.20, 0.20], # No single key has >25%
    "region_distribution": [0.33, 0.34, 0.33],             # Balanced regional traffic
    "read_write_ratio": 0.90
}


# Use itertools to cycle through all 6 profiles seamlessly
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Choose key, region, and action based on the current profile
        ...
        # Control the overall request rate
        time.sleep(1)